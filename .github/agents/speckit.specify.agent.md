...
7. Report completion with branch name, spec file path, and checklist results. Then output the following block (REQUIRED — see constitution §Speckit Workflow Continuity):
   ...
   **Recommended**: `/speckit.clarify` — review the spec for ambiguities before planning.

   If the spec is already unambiguous and all acceptance criteria are testable, skip clarify
   and proceed directly to `/speckit.plan`.
   ```

   Tailor the recommendation: if the spec contains `[NEEDS CLARIFICATION]` markers or vague
   acceptance criteria, strongly recommend `/speckit.clarify`; if it is fully resolved, indicate
   that `/speckit.plan` is safe to run immediately.
