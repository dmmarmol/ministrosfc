...
const DEFAULT_INPUT_DIR = path.resolve(__dirname, "../../../../data/imports");
...
export async function run(
  ...
  process.stdout.write(`[import] Starting import from ${flags.inputDir}/\n`);
  ...
  // ── 3. Transform ─────────────────────────────────────────────────────────────
  const { data: opponentTeams, opponentTeamMap } =
    buildOpponentTeams(historialRows);
  const { data: tournaments, tournamentMap } = buildTournaments(historialRows);
  const { players, contacts, playerMapByName, playerMapByNickname } =
  ...
  try {
    ...
    await prisma.$transaction(
      async (tx) => {
        ...
        process.stdout.write(`[import] Inserting OpponentTeams... `);
        await insertAll(tx, {
          opponentTeams,
          tournaments,
          players,
