...
export function buildGames(
  ...
  for (const row of rows) {
    ...
    const competitionType: CompetitionType = COMPETITION_TYPE_MAP[torneo] ?? "FRIENDLY";

    const status: GameStatus = COMPLETED_CONCLUSIONES.has(raw["Conclusión"]?.trim())
      ? "COMPLETED"
      : "SCHEDULED";

    const notes = assembleNotes(raw.DT, raw.Comienzo, raw["Finalización"], raw.Comentarios);
...
function assembleNotes(
  ...
  if (start?.trim() && end?.trim()) parts.push(`Horario: ${start.trim()}–${end.trim()}`);
