import { runTransitions } from "../../src/jobs/GameStatusTransitionJob";
...
describe("GameStatusTransitionJob – runTransitions", () => {
  beforeEach(() => {
  ...
  it("does not throw when prisma returns zero-count results", async () => {
    mockUpdateMany.mockResolvedValue({ count: 0 });
    await expect(runTransitions()).resolves.toBeUndefined();
  });
