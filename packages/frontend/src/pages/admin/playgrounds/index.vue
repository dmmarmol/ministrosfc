...
const { playgrounds, loading, fetchPlaygrounds, deletePlayground } = usePlaygrounds();
...
async function confirmDelete(pg: Playground) {
  if (!confirm(`¿Eliminar "${pg.name}"? Esta acción no se puede deshacer.`)) return;
  try {
        ...
        <table class="w-full text-sm text-gray-700 border-collapse">
          <thead>
            <tr class="border-b border-gray-200">
              <th class="text-left py-2 pr-3 font-medium">Nombre</th>
              <th class="text-left py-2 pr-3 font-medium">Dirección</th>
              <th v-if="authStore.isAdmin" class="text-right py-2 font-medium">Acciones</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              ...
              <td class="py-2 pr-3 text-gray-500 truncate max-w-[160px]">{{ pg.address }}</td>
              <td v-if="authStore.isAdmin" class="py-2 text-right">
            ...
            <tr v-if="!playgrounds.length">
              <td colspan="3" class="py-4 text-center text-gray-400 text-sm">No hay canchas registradas.</td>
            </tr>
          </tbody>
        </table>
      </div>
