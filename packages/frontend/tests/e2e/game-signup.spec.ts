 ...
 */
const SIGNUP_GAME_SLUG =
  process.env.E2E_SIGNUP_GAME_SLUG ?? "test-signup-game";
...
test.describe("SC-001 — Self-signup flow", () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await expect(
        page.getByText(/ya estás confirmado/i),
      ).toBeVisible();
...
test.describe("SC-002 — Guest signup flow", () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await expect(
        page.getByText(/agregar invitado/i).first(),
      ).toBeVisible();
      ...
      // Open guest form
      await page
        .getByRole("button", { name: /\+ agregar invitado/i })
        .click();

      // Fill first name and last name
      await page.getByPlaceholder(/nombre/i).fill("Test");
      await page.getByPlaceholder(/apellido/i).fill("Invitado");

      // Select a position (pick the first non-blank option)
      const positionSelect = page.locator('select').filter({ hasText: /sin posición/i });
  ...
  test("guest dismiss restores default state without creating a record", async ({
    ...
    // Guest form inputs should now be visible
    ...
    await page.getByPlaceholder(/apellido/i).fill("Be Saved");
...
test.describe("Capacity full state", () => {
  ...
  test("signup page shows capacity full message and hides signup form", async ({
    ...
    await expect(
      page.getByText(/el cupo está completo/i),
    ).toBeVisible();
