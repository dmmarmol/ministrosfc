...
| `User`      | MODIFY      | Add `onboardingCompletedAt` nullable timestamp for post-signup onboarding gate               |
...
model User {
  ...
  player          Player?   @relation(fields: [playerId], references: [id], onDelete: SetNull)
  playerId        String?   @unique
  onboardingCompletedAt DateTime?                // NEW: set after /auth/onboarding completion
  createdAt       DateTime  @default(now())
  updatedAt       DateTime  @updatedAt
  lastLoginAt     DateTime?
...
| `onboardingCompletedAt` | —                   | `DateTime?`               | Tracks whether post-signup onboarding was completed            |
...
```
[No Account] ──email register──→ [User(role=PLAYER, onboardingCompletedAt=null)]
[No Account] ──google sign-in──→ [User(role=PLAYER, onboardingCompletedAt=null, googleSubjectId=set)]
[User, onboardingCompletedAt=null] ──onboarding isPlayer=true──→ [User + Player(status=ACTIVE), onboardingCompletedAt=set]
[User, onboardingCompletedAt=null] ──onboarding isPlayer=false──→ [User(playerId=null), onboardingCompletedAt=set]
...
- Registration and Google first-login flow through one onboarding step before final player linking
- Player creation is conditional on onboarding answer (`isPlayer=true`)
