...
**Independent Test**: (1) Create a game with `date = now − 5 minutes`; `endDate` is auto-set to `date + 100 min = now + 95 min`; run the job; verify `game.status = IN_PROGRESS` and the Game Sign Up Page returns 422. (2) Create a game with `date = now − 101 minutes`; `endDate` is auto-set to `date + 100 min = now − 1 min`; run the job; verify `game.status = COMPLETED` and the game appears in the "past games" homepage section.
...
- **FR-030**: When a game is created or its `date` field is updated, the system MUST automatically compute `game.endDate = date + 100 minutes` (rounded down to the nearest minute) server-side. `endDate` MUST NOT be exposed as an editable input in any admin UI form; it is a server-only derived field. Client payloads that include `endDate` MUST be silently ignored or rejected at the schema validation layer.
...
- E8 (endDate auto-computation rule): `endDate = date + 100 minutes` (rounded down to the nearest minute), computed server-side on game create and on every update where `date` changes. The admin UI MUST NOT expose `endDate` as an editable field. Client payloads including `endDate` are ignored. Added as FR-030.
