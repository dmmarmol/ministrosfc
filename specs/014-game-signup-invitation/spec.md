...
2. **Given** the Player clicks "Cancelar inscripción", **When** the request completes, **Then** their GameParticipant record is deleted, the confirmed count decrements, the page updates without a full refresh, and the DropdownAddMore row reappears pre-filled with the player's own name.
