...
8. **Given** an Admin or Editor opens the "Create Game" form (`/admin/games/create`), **When** the form renders, **Then** the "Cupo máximo" number input is pre-filled with `16` (`DEFAULT_MAX_PLAYERS`); the input accepts only positive integers and is clearable to allow null (unlimited).
...
9. **Given** the Game Sign Up Page roster section renders, **When** `maxPlayers` is set, **Then** the section header displays "Confirmados (X/Y)" where X is the confirmed count and Y is `maxPlayers`; when `maxPlayers` is null, it displays "Confirmados (X)" with no total; the header row uses `flex justify-between` and when confirmed count is greater than or equal to `maxPlayers`, a green check heroicon and the text "Equipo completo" are displayed at the far right of that same row.
...
7. **Given** an authenticated PLAYER visits the public game detail page (`/games/{slug}`) for a `SCHEDULED` game, **When** the page renders, **Then** an "Anotarse a este partido" action link is displayed, navigating to `/games/{slug}/signup`.
8. **Given** an unauthenticated visitor or a non-PLAYER authenticated user (Admin/Editor/DT) visits the public game detail page, **When** the page renders, **Then** no signup action link is displayed.
9. **Given** the public game detail page participant section renders, **When** `maxPlayers` is set, **Then** the section header displays "Confirmados (X/Y)" where X is the confirmed count and Y is `maxPlayers`; when `maxPlayers` is null, it displays "Confirmados (X)" with no total; the header row uses `flex justify-between` and when confirmed count is greater than or equal to `maxPlayers`, a green check heroicon and the text "Equipo completo" are displayed at the far right of that same row.
...
A logged-in Player browsing the public homepage or the public schedule page sees an "Anotarse" action link directly on each SCHEDULED game card, enabling navigation to the signup page in one click. Cards reflect the Player's current signup state — already signed up shows "Ya anotado", a full game shows "Completo".
...
1. **Given** an authenticated Player visits the public homepage or the public schedule page (`/schedule`), **When** the SCHEDULED games section renders, **Then** each game card shows an "Anotarse" link navigating to `/games/:slug/signup`.
...
4. **Given** the user activates the "Agregar invitado" action within the widget, **When** the inline guest form appears, **Then** the dropdown player-list is hidden and the guest form is the only visible mode; the form includes first name (required) and an optional position `<select>` (last name field removed — T079).
...
6. **Given** the user completes the inline guest form with valid first name and optionally a position, and submits it, **When** the submission succeeds, **Then** the component emits `signup-guest(firstName, lastName, position)` with `lastName = ""`, the parent creates the guest, the new guest appears in the roster, and the guest form closes; the widget returns to its default dropdown state.
...
- **FR-042**: The public game detail page (`/games/{slug}`) MUST display an "Anotarse a este partido" action link navigating to `/games/{slug}/signup` when the authenticated caller has role `PLAYER` and `game.status = SCHEDULED`. The link MUST NOT be rendered for unauthenticated visitors, non-PLAYER authenticated users (Admin/Editor/DT), or games with any status other than `SCHEDULED`. The link MUST be positioned between the game header card and the confirmed-players roster section.
- **FR-043**: The game creation form (`/admin/games/create`) MUST include a "Cupo máximo" `<input type="number">` field that pre-fills with `DEFAULT_MAX_PLAYERS` (16) when the form first renders. The input MUST enforce `min="1"` and `step="1"` to accept only positive integers. Clearing the field MUST submit `null` (unlimited capacity). The `maxPlayers` value MUST be included in the `POST /api/v1/games` request body.
- **FR-044**: Both the public game detail page (`/games/{slug}`) and the Game Sign Up Page (`/games/{slug}/signup`) MUST render the confirmed-players section header as a `flex justify-between` row. The left side MUST display "Confirmados (X/Y)" when `maxPlayers` is set (where X is the current confirmed count and Y is `maxPlayers`), or "Confirmados (X)" when `maxPlayers` is null. When the confirmed count is greater than or equal to `maxPlayers`, the right side of the same row MUST display a green check heroicon (Heroicons `check-circle` solid, green) alongside the text "Equipo completo"; this indicator MUST appear on both pages and be updated reactively on the signup page without a full refresh.
...
- `DEFAULT_MAX_PLAYERS = 16` is defined in `@ministrosfc/shared/src/constants/games.ts` and used as the default value for the game creation form and as the capacity fallback constant throughout the frontend. Existing games with `maxPlayers = null` are unaffected by this default — null means unlimited; the constant is only applied at creation time in the UI.
