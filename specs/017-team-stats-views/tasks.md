...
- [x] T022–T026c [Retro] **Path rename**: All `components/pages/stats/` and `pages/stats/` path references in Phases 5–7 diverged from the final implementation. The component directory was renamed `stats/` → `statistics/` and all pages were moved from `src/pages/stats/` to `src/pages/statistics/`. All sub-components under `StatsFilters/` were also renamed from `*Filter.vue` to `*Select.vue` (e.g. `PlayerFilter.vue` → `PlayerSelect.vue`). Change documented per Amendment 2026-04-20.
...
- [x] T031b [Retro] **`mode` prop removed**: `StatsTableByRival.vue` implementation diverged from T031. The `mode: 'all' | 'single'` prop and the dual-column-set behaviour were dropped. The component renders a single flat column set and is only used by `rivals/index.vue`. The `rivals/[id].vue` route uses a separate dedicated rival detail page without `StatsTableByRival`. Change documented per Amendment 2026-04-20.
...
- [x] T033b [Retro] **Test scope simplified**: Unit test for `StatsTableByRival` was updated in line with T031b — mode-specific column tests removed; final suite covers: renders header, maps rows correctly, shows empty state (3 tests).
...
- [x] T034b [Retro] **`goalRate` column label is `"PG"`** (Promedio de Gol): the column header renders as the abbreviated label `"PG"` in both `allColumns` and `singleColumns`. The `winRate` field is delivered by the API as a percentage float (e.g. `60.0`), NOT as a decimal fraction — components render it directly with `.toFixed(1)%` without any `* 100` multiplication. Change documented per Amendment 2026-04-20.
